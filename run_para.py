#!/opt/python/3.x/3.4.3_intel/build2/bin/python3
import qgl
import qgl_util
import qgl_plotting
...
IC_list = []
gradient_operator = '1'
hamiltonian_types = ['NN_0','NN_1','NN_2','NN_01','NN_02','NN_12']
...
# Simulations to Run
# ------------------
simulations = [ (tasks,  IC[0], t_span, dt, IC[1:], output_dir, gradient[0], gradient[1] , gradient[2], l, restart, lmax, hamiltonian_type, V)
                     for dt       in dt_list
...
if post_processing:
    ...
    for i, simulation in enumerate(simulations):
        if i % nprocs == rank:
            qgl_plotting.main(output_dir, simulation[0], simulation[3], simulation[2], simulation[1], 
                              simulation[4], simulation[-2], simulation[-1], gradients = gradients)
